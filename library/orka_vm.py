    ...
    while time.time() - start_time < timeout:
        ...
        if not current_vm:
            if desired_state == 'absent':
                return True
            module.fail_json(
                msg=f"VM '{name}' not found while waiting for state change",
                **result
            )
...
def start_vm(module, name, binary_path, result, wait_timeout=60):
    ...
    try:
        ...
        if not wait_for_vm_state(module, name, 'running', binary_path, result, wait_timeout):
            module.fail_json(
                msg=f"Timeout waiting for VM '{name}' to start",
                **result
            )
...
def stop_vm(module, name, binary_path, result, wait_timeout=60):
    ...
    try:
        ...
        if not wait_for_vm_state(module, name, 'stopped', binary_path, result, wait_timeout):
            module.fail_json(
                msg=f"Timeout waiting for VM '{name}' to stop",
                **result
            )
    except subprocess.CalledProcessError as e:
        module.fail_json(
...
def main():
    ...
    if not existing_vm and desired_state == 'absent':
        exit_with_result(False, 'absent', f"VM '{name}' does not exist, nothing to delete")
    
    if not existing_vm and desired_state != 'absent':
        module.fail_json(
            msg=f"VM '{name}' does not exist",
            **result
        )
